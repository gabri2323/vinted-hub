...
export async function ensureSchema() {
  ...
  //   sociales  -> a quién ha seguido / escrito cada PC (para que NINGÚN perfil de
  //                ningún equipo repita a la misma persona: lista de sistema compartida)
  //   mensajes  -> mapa {item_id -> mensaje al que da like} COMÚN (clave 'GLOBAL', merge):
  //                la casilla del Comparador es la misma en los dos PC
  ...
  await sql`CREATE TABLE IF NOT EXISTS sociales (pc_id text PRIMARY KEY, data text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now())`;
  await sql`CREATE TABLE IF NOT EXISTS mensajes (pc_id text PRIMARY KEY, data text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now())`;
  // MOVIL: cola de subidas de un amigo (foto+perfiles) que espera a que el PC la
  // procese (Gemini -> propuesta). `movil_perfiles` guarda la lista de los 7
  // perfiles que el PC publica, para que la pagina publica los muestre. Ver
  // api/subir.js, public/subir.html y app/movil_hub.py.
  await sql`
    CREATE TABLE IF NOT EXISTS movil_cola (
      id        bigserial PRIMARY KEY,
      data      text NOT NULL,
      creado_en timestamptz NOT NULL DEFAULT now()
    )
  `;
  await sql`CREATE TABLE IF NOT EXISTS movil_perfiles (pc_id text PRIMARY KEY, data text NOT NULL, updated_at timestamptz NOT NULL DEFAULT now())`;
  // TURNO GLOBAL de Dolphin: una sola fila (id='global') que dice que PC puede
  // tener un perfil abierto ahora. El limite de "un perfil a la vez" es de la
  // CUENTA, no del ordenador, asi que el candado tiene que ser compartido; el
  // de cada PC no ve al otro. Caduca solo (`hasta`) para que un PC colgado no
  // deje al otro esperando para siempre. Ver api/turno.js y app/hub.py.
  await sql`
